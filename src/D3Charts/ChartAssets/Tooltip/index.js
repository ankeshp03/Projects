...
let Tooltip = (event, data, options) => {
    ...
    .html(
      `<div class='container pT5 pB5'>
        ...
        (options.customRenderer
          ? options.customRenderer({
              headerContainerClass: "tooltipHeader",
              headerLabelClass: "tooltipHeader-label",
              headerValueClass: "tooltipHeader-value",
              valueContainerClass: "tooltipValue",
              valueLabelClass: "tooltipValue-label",
              valueValueClass: "tooltipValue-value"
            })
          : hasLabel
          ? `<p class='tooltipHeader mB0'>
                            <span class='tooltipHeader-label'>${
                              data?.displayKey?.label
                            }: </span>
                            <span class='tooltipHeader-value'>${
                              data?.displayKey?.value ?? ""
                            }</span>
                            </p>
                            <p class='tooltipValue mB0'>
                                <span class='tooltipValue-label'>${
                                  data?.displayValue.label
                                }: </span>
  ...
  let screenWidth = window.innerWidth,
    ...
    tooltipWidth = document.querySelector(`.${options?.className}`)
      ?.clientWidth,
    tooltipHeight = document.querySelector(`.${options?.className}`)
      ?.clientHeight;
